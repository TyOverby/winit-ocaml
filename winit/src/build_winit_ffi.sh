...
if [ "$PROFILE" = "release" ]; then
  cargo build --quiet --release -p winit_ffi --manifest-path $SOURCE_ROOT/Cargo.toml
  ...
  cp $SOURCE_ROOT/_build/rust/release/libwinit_ffi.$DYLIB_EXT $OUT_DIR/libwinit_ffi.so
else
  cargo build --quiet -p winit_ffi --manifest-path $SOURCE_ROOT/Cargo.toml
  ...
  # Also copy with lib prefix for native dynamic linking
  cp $SOURCE_ROOT/_build/rust/debug/libwinit_ffi.$DYLIB_EXT $OUT_DIR/libwinit_ffi.so
fi
