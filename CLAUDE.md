...
2. Using the `yml` file, write a generator that produces high-level, idiomatic
   ocaml code which calls out to the APIs in the low-level bindings.
...
Build test executables and run them regularly.  (These tests may need to be
headless, you won't always have access to a display driver).  If you can have
a headless binary produce `.png` files that you can read, that's excellent!
