...
CAMLprim value caml_wgpu_adapter_request_device_sync(value instance_val, value adapter_val) {
...
CAMLprim value caml_wgpu_adapter_get_info(value adapter_val) {
  ...
  const char *empty = "";
  const char *vendor_ptr = (info.vendor.data != NULL && info.vendor.length > 0) ? info.vendor.data : empty;
  const char *arch_ptr = (info.architecture.data != NULL && info.architecture.length > 0) ? info.architecture.data : empty;
  const char *device_ptr = (info.device.data != NULL && info.device.length > 0) ? info.device.data : empty;
  const char *desc_ptr = (info.description.data != NULL && info.description.length > 0) ? info.description.data : empty;
...
CAMLprim value caml_wgpu_command_encoder_begin_render_pass_with_depth(
  ...
  if (Is_block(resolve_target_opt_val) && Tag_val(resolve_target_opt_val) == 0) {
    resolve_target =
