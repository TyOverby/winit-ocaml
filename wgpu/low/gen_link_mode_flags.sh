...
case "$(uname -s)" in
  Darwin)
    ...
    if [ "$PROFILE" = "release" ]; then
      ...
      echo "(\"$CURRENT_DIR/libwgpu_native.a\")" > link_mode_flags.sexp
    else
      # Dynamic linking - macOS uses .dylib and different rpath syntax
      echo "(-L$CURRENT_DIR -lwgpu_native -Wl,-rpath,$CURRENT_DIR)" > link_mode_flags.sexp
    fi
    ;;
  *)
    # Linux and others
    if [ "$PROFILE" = "release" ]; then
      ...
      echo "(-L$CURRENT_DIR -l:libwgpu_native.a)" > link_mode_flags.sexp
    else
      ...
      echo "(-Wl,-Bdynamic -L$CURRENT_DIR -lwgpu_native -Wl,-rpath,$CURRENT_DIR)" > link_mode_flags.sexp
    fi
    ;;
esac
