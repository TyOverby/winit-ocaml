...
@vertex fn vs(vert: Vertex) -> VSOutput {
  ...
  // The rotation values come directly from a point on the unit circle.
  // For angle theta: rotation.x = cos(theta), rotation.y = sin(theta)
  //
...
let init () =
  ...
  let shader =
    Wgpu.Device.create_shader_module
      device
      ~label:"rotation_unit_circle_shader"
      ~wgsl:shader_code
      ()
  in
...
let render
  ...
  in
  (* Color: teal *)
  Bigarray.Array1.set uniform_data 0 0.2;
  Bigarray.Array1.set uniform_data 1 0.8;
  Bigarray.Array1.set uniform_data 2 0.8;
  ...
  (* Rotation: direct unit circle coordinates *)
  Bigarray.Array1.set uniform_data 8 unit_circle_x;
  Bigarray.Array1.set uniform_data 9 unit_circle_y;
  (* Padding *)
...
let () =
  ...
  let pipeline_layout =
    Wgpu.Device.create_pipeline_layout
      device
      ~label:"rotation_unit_circle_pipeline_layout"
  ...
  (* Render at different positions on the unit circle.
     ...
     - 90 degrees: (0, 1) - quarter turn *)
  let deg_to_rad d = d *. Float.pi /. 180.0 in
  let translation_x = 200.0 in
  let translation_y = 150.0 in
  let angles =
    [ 0.0, "rotation_unit_circle_0deg"
    ; 30.0, "rotation_unit_circle_30deg"
    ; 60.0, "rotation_unit_circle_60deg"
    ; 90.0, "rotation_unit_circle_90deg"
    ]
  in
  List.iter angles ~f:(fun (degrees, output_name) ->
    let angle = deg_to_rad degrees in
    let unit_circle_x = Float.cos angle in
    let unit_circle_y = Float.sin angle in
    render
      ~device
      ~queue
      ~pipeline
