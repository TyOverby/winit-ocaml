...
(* Low resolution to demonstrate the fix *)
let width = 40
let height = 40
...
let () =
  ...
  in
  let ( (* Write output *) ) =
    let ppm_file = Test_util.output_path "multisample_centroid.ppm" in
    let png_file = Test_util.output_path "multisample_centroid.png" in
