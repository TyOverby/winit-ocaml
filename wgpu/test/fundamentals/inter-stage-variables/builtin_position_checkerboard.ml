...
@fragment fn fs(fsInput: OurVertexShaderOutput) -> @location(0) vec4f {
  ...
  // fsInput.position.xy gives pixel coordinates
...
let () =
  ...
  let pipeline =
    Wgpu.Device.create_render_pipeline
      device
      ~label:"builtin_position_pipeline"
      ...
      ()
  in
  render ~device ~queue ~pipeline ~output_name:"builtin_position_checkerboard";
  (* Cleanup *)
