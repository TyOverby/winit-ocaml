(* WebGPU Fundamentals: Vertex Buffers with Per-Vertex Colors
   ...
   The final color is computed as: instance_color * per_vertex_color
*)
...
(* Create circle vertices with per-vertex colors (position + rgb) *)
let create_circle_vertices ~radius ~inner_radius =
  (* 2 triangles per subdivision, 3 verts per tri, 5 values (xy + rgb) each *)
  ...
  let add_vertex x y r g b =
    ...
    offset := !offset + 5
  in
  let inner_color = 1.0, 1.0, 1.0 in
  let outer_color = 0.1, 0.1, 0.1 in
  for i = 0 to num_subdivisions - 1 do
    ...
    let or_, og, ob = outer_color in
    let ir, ig, ib = inner_color in
    (* first triangle *)
    add_vertex (c1 *. radius) (s1 *. radius) or_ og ob;
    add_vertex (c2 *. radius) (s2 *. radius) or_ og ob;
    add_vertex (c1 *. inner_radius) (s1 *. inner_radius) ir ig ib;
    (* second triangle *)
    add_vertex (c1 *. inner_radius) (s1 *. inner_radius) ir ig ib;
    add_vertex (c2 *. radius) (s2 *. radius) or_ og ob;
    add_vertex (c2 *. inner_radius) (s2 *. inner_radius) ir ig ib
  done;
...
let init () =
  ...
  let shader =
    Wgpu.Device.create_shader_module
      device
      ~label:"vertex_buffers_per_vertex_colors_shader"
      ~wgsl:shader_code
      ()
  in
...
let () =
  ...
  let vertex_buffer_layout_static =
    Wgpu.Vertex_buffer_layout.create
      ~step_mode:Wgpu.Vertex_step_mode.Instance
      ~array_stride:(Int64.of_int static_unit_size)
      ~attributes:
        [ Wgpu.Vertex_attribute.create
            ~format:Wgpu.Vertex_format.Float32x4
            ~offset:0L
            ~shader_location:1
            ()
        ; Wgpu.Vertex_attribute.create
  ...
  let pipeline =
    Wgpu.Device.create_render_pipeline
      device
      ~label:"vertex_buffers_per_vertex_colors_pipeline"
  ...
  let render_pass =
    Wgpu.Command_encoder.begin_render_pass
      encoder
      ~label:"vertex_buffers_per_vertex_colors_pass"
      ~color_attachments:
  ...
  in
  let ( (* Write output *) ) =
    let ppm_file = Test_util.output_path "vertex_buffers_per_vertex_colors.ppm" in
    let png_file = Test_util.output_path "vertex_buffers_per_vertex_colors.png" in
