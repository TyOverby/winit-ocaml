...
(* ===== run_offset tests ===== The contract: [run_offset grid out w h ~ox ~oy] is
   identical to [run] except that every emitted coordinate is shifted by (ox, oy). In
   particular, marching a tile sub-grid with the tile's global origin as the offset must
   produce segments bitwise-identical to the corresponding segments of a dense [run] over
   the full grid — which is what lets [line_join] stitch segments across tile seams by
...
let%expect_test "run_offset translates segments by (ox, oy)" =
  (* A 2x2 one-cell grid with one negative corner: the segment is at (0.5, 0) -> (0, 0.5)
     under run. With ox=10 oy=20 it should be (10.5, 20) -> (10, 20.5). *)
  let input : float32# array = [| Float32_u.neg #1.0s; #1.0s; #1.0s; #1.0s |] in
  let output = make_output ~width:2 ~height:2 in
...
let%expect_test "run_offset: tile of full grid yields bitwise-identical segments" =
  (* Build a 4x4 full grid with a vertical boundary at x=2. Then extract the left 3x4 tile
     (ox=0,oy=0) and the right 3x4 tile (ox=2,oy=0), each from their respective
     sub-arrays. Segments shared at x=2 must be bitwise identical between the two partial
     runs and the full run. *)
  let w = 4
  and h = 4 in
  let full =
    make_grid ~width:w ~height:h ~f:(fun ~x ~y:_ ->
  ...
  let left_w = 3 in
  let left = make_grid ~width:left_w ~height:h ~f:(fun ~x ~y -> full.((y * w) + x)) in
  ...
  let right_w = 2 in
  let right =
    make_grid ~width:right_w ~height:h ~f:(fun ~x ~y -> full.((y * w) + (x + 2)))
  in
  ...
  (* The boundary segments (at x=1.5 in local coords, or x=2..2 in global) are the ones
     emitted by both the left tile and the full run at x=1.5 local = x=1.5 in full (column
     1->2 boundary). Print them for comparison. *)
...
let%expect_test "run_offset: large offset preserves segment structure" =
  (* A 3x3 diagonal-boundary grid run at offset (100, 200). Segment coordinates should
     equal the zero-offset coords + (100, 200). *)
  let w = 3
  and h = 3 in
  let input =
    make_grid ~width:w ~height:h ~f:(fun ~x ~y ->
  ...
  (* Every segment from the offset run should equal the zero-offset run's coordinates
     shifted by (100, 200). *)
  let all_shifted =
    count0 = countN
    &&
...
let%expect_test "run_offset: bitwise identity - tile boundary shared vertex" =
  (* The doc says: 'marching a tile of a larger grid produces segments bitwise identical
     to the corresponding segments of a dense run over the whole grid.' Verify this for
     the shared edge at column 2 of a 4-wide grid. *)
  let w = 4
  and h = 3 in
  let full =
    make_grid ~width:w ~height:h ~f:(fun ~x ~y:_ ->
  ...
  let tile_w = 2 in
  let tile =
    make_grid ~width:tile_w ~height:h ~f:(fun ~x ~y -> full.((y * w) + (x + 2)))
  in
  let out_tile = make_output ~width:tile_w ~height:h in
  let count_tile = March.run_offset tile out_tile tile_w h ~ox:2 ~oy:0 in
  (* Find the segments at x1=1.5 (full) and x1=1.5 (tile with ox=0) — actually tile at
     ox=2 has no boundary because both columns are positive. The left tile: columns 0..2
     run at offset ox=0 *)
  let ltile_w = 3 in
  let ltile = make_grid ~width:ltile_w ~height:h ~f:(fun ~x ~y -> full.((y * w) + x)) in
  ...
  (* The boundary segments from full and left-tile must be bitwise identical. Both emit
     vertical segments at x=1.5, crossing from y=0 to y=2. *)
