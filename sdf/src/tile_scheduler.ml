...
(* The per-task state of the subdivision: everything an interval evaluation of a rectangle
   needs. On this branch [Expr_graph_range_eval.run] allocates its working buffers per
   call, so the state is immutable and [create] is free; it exists so that every parallel
   task visibly creates its own state at its top — when the range evaluator grows a
   mutable per-evaluation context, [create] is the one place that changes. *)
module Task_state = struct
  ...
  let create range ~vars ~oracles = { range; vars; oracles }

  let eval t ~x ~y =
    Expr_graph_range_eval.run t.range ~vars:t.vars ~oracles:t.oracles ~x ~y
  ;;
end
...
let rec descend_par
  ...
  (* Every task gets its own interval-evaluation state. *)
