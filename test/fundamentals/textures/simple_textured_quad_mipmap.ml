...
let upload_mip_levels ~device ~queue ~texture ~mips =
  List.iteri mips ~f:(fun mip_level (mip_data, mip_width, mip_height) ->
    (* Align bytes_per_row to 256 bytes as required by WebGPU *)
    let src_bytes_per_row = mip_width * 4 in
    let aligned_bytes_per_row = (src_bytes_per_row + 255) / 256 * 256 in
    let staging_buffer_size = aligned_bytes_per_row * mip_height in
    let staging_buffer =
      Wgpu.Device.create_buffer
        device
        ~label:(sprintf "mip_%d_staging_buffer" mip_level)
    ...
    in
    for y = 0 to mip_height - 1 do
      for x = 0 to (mip_width * 4) - 1 do
        let src_offset = (y * src_bytes_per_row) + x in
        let dst_offset = (y * aligned_bytes_per_row) + x in
        Bigarray.Array1.set
          aligned_data
          dst_offset
          (Bigarray.Array1.get mip_data src_offset)
      done
    done;
    ...
    let copy_encoder =
      Wgpu.Device.create_command_encoder
        device
        ~label:(sprintf "mip_%d_copy_encoder" mip_level)
        ()
    in
    Wgpu.Command_encoder.copy_buffer_to_texture
      ...
      ~source_layout_rows_per_image:mip_height
      ...
      ~copy_size_width:mip_width
      ~copy_size_height:mip_height
      ~copy_size_depth_or_array_layers:1
      ();
    let copy_commands =
      Wgpu.finish copy_encoder ~label:(sprintf "mip_%d_copy_commands" mip_level) ()
    in
