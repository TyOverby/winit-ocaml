...
(* This test demonstrates that bigarray functions work with different element types,
   not just int8_unsigned. We write float32 data to a GPU buffer and read it back,
...
let () =
  ...
  (* Write float32 data directly to GPU buffer - this demonstrates
     Queue.write_buffer accepting float32 bigarray *)
  ...
  (* Map and read back as float32 - this demonstrates get_const_mapped_range
     with kind:Bigarray.float32 *)
  let output_data =
    Wgpu.map_buffer
      readback_buffer
