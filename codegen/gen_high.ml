...
let gen_ml (api : Ir.api) : string =
  ...
  (* Filter out Instance since we generate a custom one *)
  let objects =
    List.filter api.objects ~f:(fun obj -> not (String.equal obj.name "instance"))
...
|}
  in
  String.concat [ header; enums; bitflags; objects; instance_module ]
;;
